...
use secp256k1::{Secp256k1, rand::{thread_rng, RngCore}};
use sha2::Sha256;
...
impl ExtProgressBar {
    ...
    fn update(&self, total_count: u64) {
        ...
        self.pb.set_message(&format!("{}/{} estimated tries", position_msg, self.total_msg));
    }
...
fn run(id: usize, prefix: String, vanity: String, count: Arc<AtomicU64>, success: Arc<AtomicBool>, epb: Arc<ExtProgressBar>) {
    // Contexts
...
fn main() {
    ...
    let matches = App::new("Witnet vanity address generator")
        ...
        .about("Vanity address generator using curve Secp256k1 and in Bech32 format: <hrp>1<string>")
        .author("Stampery Labs")
        .arg(Arg::with_name("vanity-string")
            .help("Vanity prefix string to generate")
            .index(1)
            .required(true))
        .arg(Arg::with_name("hrp")
            .long("hrp") // allow --hrp
            .short("H")
            .takes_value(true)
            .default_value("wit")
        ...
        .arg(Arg::with_name("threads")
            ...
            .short("t")
            .takes_value(true)
            .help("Number of running threads executed in parallel [default: threads = num_cpus]")
            .required(false))
        .get_matches();

    // Extract the actual name
    let vanity = matches.value_of("vanity-string")
        .expect("Vanity string is required");

    // Extract the hrp
    let hrp = matches.value_of("hrp")
        .expect("Human-readable part should be set");

    // Extract number of cpus to be used
    let num_cpus = num_cpus::get();
    let num_threads = match matches.value_of("num-cpus") {
        None => num_cpus,
    ...
    for c in vanity.chars() {
        if !BECH32_CHARSET.contains(&c) {
            eprintln!("Vanity string contains the invalid character `{}`.\n\
                      Only Bech32 characters are allowed {:?}.", c, BECH32_CHARSET);
            return;
        }
    }

    // Show info and progress bar
    println!("\nSearching vanity addresses with the prefix: {}{}{}\n", hrp, BECH32_SEPARATOR, vanity);
